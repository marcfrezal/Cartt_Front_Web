...
const SelectBrand = (props) => {
  ...
  if (loading) {
    ...
    if ( data.me.brands.length === 0) {
      props.selectBrand(data.me.brands[0]._id);
      return (
...
const SwitchUserModal = (props) => {
  ...
  if (loading) {
    ...
    return (
      ...
      return (
        <Col className="headerSwitch" xs={12} sm={5} md={5} lg={3}>
          {data.me.currentBrand !== null ? <div className="headerSwithBrand">{data.me.currentBrand.name}</div> : <div className="headerSwithBrand">Veuillez choisir une marque</div> }
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
              <Form.Label>Choisissez une marque ci-dessous sur laquelle basculer afin d'en visualiser les données relatives sur votre espace administrateur : </Form.Label>
    ...
    return (
      <Col className="headerSwitch" xs={12} sm={5} md={5} lg={3}>
        <p className="headerSwithBrand">Pas de marques liées.</p>
      </Col>
    )
  }
};
